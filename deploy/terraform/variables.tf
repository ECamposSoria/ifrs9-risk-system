...
variable "vertex_region" {
  description = "Primary region for Vertex AI resources"
  type        = string
  default     = "southamerica-east1"
}

variable "vertex_zone" {
  description = "Zone used for Vertex AI Workbench instances"
  type        = string
...
variable "enable_gke" {
  description = "Provision the GKE cluster and related resources"
  type        = bool
  default     = true
}

variable "enable_composer" {
  description = "Provision the Cloud Composer environment"
  type        = bool
  default     = true
}

variable "enable_dataproc" {
  description = "Provision the Dataproc cluster"
  type        = bool
  default     = true
}

variable "enable_backup_services" {
  description = "Provision backup services (GKE backup plan and Storage Transfer jobs)"
  type        = bool
  default     = true
}

variable "enable_serverless_orchestration" {
  description = "Provision the Cloud Run + Workflows based orchestration stack"
  type        = bool
  default     = false
}

variable "enable_autopilot_drain_guard" {
  description = "Block subnet updates while Autopilot maintenance nodes (gk3-*) are still running"
  type        = bool
  default     = true
}
...
variable "storage_bucket_retention_days" {
  description = "Retention period enforced on managed Cloud Storage buckets"
  type        = number
  default     = 365
}
...
variable "serverless_region" {
  description = "Region for serverless orchestration resources"
  type        = string
  default     = "us-central1"
}

variable "serverless_schedule_cron" {
  description = "Cron schedule used by Cloud Scheduler to trigger the workflow"
  type        = string
  default     = "0 2 * * *"
}

variable "serverless_cloud_run_image" {
  description = "Container image executed by the Cloud Run service"
  type        = string
  default     = "us-docker.pkg.dev/cloudrun/container/hello"
}

variable "serverless_timeout_seconds" {
  description = "Request timeout in seconds for the Cloud Run service"
  type        = number
  default     = 900
}

variable "ifrs9_batch_job" {
  description = "Configuration for the IFRS9 Cloud Run batch job"
  type = object({
    record_count = number
    seed         = number
    gcs_bucket   = string
...
variable "enable_vertex_workbench" {
  description = "Provision Vertex AI Workbench instance"
  type        = bool
  default     = false
}
...
variable "billing_account_id" {
  description = "Billing account identifier for budget monitoring"
  type        = string
  default     = ""
}

variable "budget_monthly_amount" {
  description = "Monthly budget amount (same currency as billing account)"
  type        = number
  default     = 0
}

variable "budget_alert_spend_threshold" {
  description = "Spend threshold (0-1) that triggers budget alerts"
  type        = number
  default     = 0.8
}

variable "budget_currency_code" {
  description = "Currency code used for budget calculations"
  type        = string
  default     = "USD"
}
...
variable "bigquery_pipeline_writers_group" {
  description = "Google group email with write access to core BigQuery datasets"
  type        = string
  default     = ""
}

variable "bigquery_dashboard_viewers_group" {
  description = "Google group email with read access to analytics datasets"
  type        = string
  default     = ""
}
